...
def plot_init(ticker:str, benchmark_ticker:str, risk_free_rate:float, start_date:list, end_date:list, 
    ...
    cells=dict(values=[["Ticker", "Benchmark Ticker", "Risk-free Rate", "Start Date", "End Date", 
               ...
               line_color='darkslategray', fill_color='lightgoldenrodyellow', align='center')),
                 row=1, col=1)    
    fig.update_layout(width=700, height=700, title_text="<span style='font-size: 30px;'><b>Initial Configuration</b></span>", title_x=0.5)
    ...
    if show_tables == True:
        ...
        fig.add_trace(go.Table(columnwidth=[3, 1],
                               ...
                               cells=dict(values=[[
                                ...
                                line_color='darkslategray', fill_color='lightcyan', align='center')),
                                row=3, col=2)
        fig.update_layout(width=1000, height=1000, title_text="<span style='font-size: 30px;'><b>BACKTEST RESULTS</b></span>", title_x=0.5)
        st.plotly_chart(fig, use_container_width=True)
...
def plot_charts(ticker: str, ohlcv: pd.DataFrame, predictions: np.array, portfolio_value: np.array, liquidated:bool) -> None:
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=False, vertical_spacing=0.1, 
                        subplot_titles=(f"<span style='font-size: 30px;'><b>OHLC of {ticker}</b></span>", f"<span style='font-size: 30px;'><b>Volume of {ticker}</b></span>"), row_width=[1, 5])
    ...
    fig.update_layout(title="<span style='font-size: 30px;'><b>Portfolio Value</b></span>", title_x=0.5)
    st.plotly_chart(fig, use_container_width=True)
...
def metric_optimization(metric:str, take_profit_value:float, stop_loss_value:float, leverage_value:float): 
    ...
    benchmark_ticker:str = st.session_state["backtest_configuration"]["benchmark_ticker"]
    ...
    initial_capital:float = st.session_state["backtest_configuration"]["initial_capital"]
    ...
    trailing_take_profit:bool = st.session_state["backtest_configuration"]["trailing_take_profit"]
    take_profit:float = take_profit_value
    trailing_stop_loss:bool = st.session_state["backtest_configuration"]["trailing_stop_loss"]
...
def optimize_backtest(metric_optimized:str, take_profit_values:list, stop_loss_values:list, leverage_values:list, iteration:int, 
                      best_n:int = 3, verbose:bool = True, show_results:bool = True):
    start = time.time()
    combinations = np.array(np.meshgrid(take_profit_values, stop_loss_values, leverage_values)).T.reshape(-1,3)
    ...
    for combination in combinations:
        ...
        if verbose == True:
            t.markdown(f"{combination_count}/{len(combinations)} (%{round(combination_count/len(combinations)*100,2)}) of combinations were tested. {second_2_minute_converter(time.time()-start)} passed.")
    ...
    if show_results == True:
        ...
        fig.add_trace(go.Table(columnwidth=[1, 1, 1, 1],
                               header=dict(values=["Take Profit", "Stop Loss", "Leverage", f"{metric_optimized}"], 
                               line_color='darkslategray', fill_color='lightskyblue', align='center'),
                               cells=dict(values=[tf, sl, lv, om], line_color='darkslategray', fill_color='lightcyan', 
                               align='center')), row=1, col=1)   
        fig.update_layout(width=1000, height=1000, title_text="<span style='font-size: 30px;'><b>OPTIMIZATION RESULTS</b></span>", title_x=0.5)
        st.plotly_chart(fig, use_container_width=True)
    end = time.time()
    if verbose == True:
