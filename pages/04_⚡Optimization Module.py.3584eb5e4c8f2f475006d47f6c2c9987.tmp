...
if st.session_state["ohlcv"] is None or st.session_state["predictions"] is None or st.session_state["ticker"] == "" or st.session_state["backtest_configuration_ready"] == False:
    ...
    metric_optimized = st.selectbox("Please select the backtest metrics to optimize:", ["<Select>", "Total Return",
                                    "Sharpe Ratio", "Sortino Ratio", "Omega Ratio"])
    st.markdown("<br>", unsafe_allow_html=True)
    take_profit_ranges = st.slider('Select a range for take profit values', 0, 50, (5, 25))
    ...
    with col1:
        iteration = int(st.number_input("Choose the iteration number to try optimized values.", value = 0))
    with col2:
        best_n = int(st.number_input("Choose the number of best combinations to see.", value = 0))
    col1, col2 = st.columns([1,1])
    with col1:
    ...
    if metric_optimized != "<Select>" and iteration != 0 and best_n != 0:
        if st.button("Run the optimization"):
            optimize_backtest(metric_optimized = metric_optimized, take_profit_values = take_profit_values, 
                            stop_loss_values = stop_loss_values, leverage_values = leverage_values, 
                            iteration = iteration, best_n = best_n, verbose = verbose, show_results = show_results)
